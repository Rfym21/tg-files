...
- `store/`: `ObjectStore` translates S3/WebDAV operations to metadata + Telegram calls.
  ...
  - Each PUT uploads as a **single** Telegram document — no upload-side chunking. The single-file ceiling is derived from the Telegram endpoint by `config.ResolveMaxUploadSize()`: 50 MB on the public Bot API (`api.telegram.org`), 2000 MiB on a self-hosted `--local` server. Oversized PUTs return `ErrEntityTooLarge`. Note: the **download** path (`GetObject`, `SelectChunksForRange`) still reads multi-chunk objects — historical data and S3 multipart objects span several chunks (one per part).
